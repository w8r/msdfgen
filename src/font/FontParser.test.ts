...
describe("FontParser", () => {
  ...
  beforeAll(async () => {
    ...
    if (fs.existsSync(robotoPath)) {
      ...
      robotoBuffer = buffer.buffer.slice(
        buffer.byteOffset,
    ...
    if (fs.existsSync(woff2Path)) {
      ...
      woff2Buffer = buffer.buffer.slice(
        buffer.byteOffset,
        buffer.byteOffset + buffer.byteLength,
      );
    }
  ...
  describe("compound glyphs (winding order)", () => {
    it.each(TEST_CHARACTERS.counters)(
      'should handle counter glyph "%s" correctly',
      async (char) => {
        if (!robotoBuffer) return;
        const font = await parseFont(robotoBuffer);
  ...
  describe("accented characters (composite glyphs)", () => {
    it.each(TEST_CHARACTERS.accented)(
      'should handle accented "%s" correctly',
      async (char) => {
        if (!robotoBuffer) return;
        const font = await parseFont(robotoBuffer);
  ...
  describe("glyph metrics", () => {
    it("should return correct advance width", async () => {
      ...
      expect(glyphM!.metrics.advanceWidth).toBeGreaterThan(
        glyphI!.metrics.advanceWidth,
      );
    });
